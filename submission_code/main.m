...
clc
res_all = [];
addpath('H:\5. pinball svdd\classical_data')
for data_num = 20
    ...
    %%
    %     outlier_mean = mean(outlier_data(:,1:end-1));
    %     outlier_std = std(outlier_data(:,1:end-1));
    %     noise_generation =[];
    %     for num_dim=1:length(outlier_mean)
    %         tem_dim = outlier_std(1,num_dim).*randn(size(normal_data,1),1)+outlier_mean(1,num_dim);
    %         noise_generation=[noise_generation,tem_dim];
    %     end
    %     noise_generation = [noise_generation,ones(size(normal_data,1),1)];
    %     noise_rate = [0.1:0.1:1];
    %     noise_rate = 0;
    %     for rate = 1:length(noise_rate)
    %         %
    %         numnoise = ceil(noise_rate(rate)*size(normal_data,1));
    %         noise = noise_generation(randperm(size(normal_data,1),numnoise),:);
            ...
            for s = 1:length(s_array)
                ...
                for f = 1:5
                    ...
                    for sigm = 1:length(sigm_array)
                        ...
                        pinballsvddParmeter = struct('nu',v_array,'tao',t_array(t),'svalue',s_array(s), 'theta', theta_value, 'kernelFunc',kernel);
                    ...
                    gmean1(f) = val_gmean(val_ind); aupr1(f) = val_aupr(val_ind);
                    ...
                    gmean2(f) = test_gmean(val_ind); aupr2(f) = test_aupr(val_ind);
                ...
                res.f1fold = f12; res.gmfold = gmean2;
                ...
                trainf1_mean = mean(f11); traingmean_mean = mean(gmean1); trainaupr_mean = mean(aupr1);
                trainsen_mean = mean(sen1); trainspe_mean = mean(spe1); trainrec_mean = mean(rec1); trainpre_mean = mean(pre1); time_mean = mean(time);
                train_result1 = [trainsen_mean trainspe_mean trainrec_mean trainpre_mean trainerrorrate_mean trainacc_mean trainf1_mean trainauc_mean traingmean_mean trainaupr_mean time_mean];
                ...
                trainf1_std = std(f11); traingmean_std = std(gmean1); trainaupr_std = std(aupr1);
                trainsen_std = std(sen1); trainspe_std = std(spe1); trainrec_std = std(rec1); trainpre_std = std(pre1); time_std = std(time);
                train_result2 = [trainsen_std trainspe_std trainrec_std trainpre_std trainerrorrate_std trainacc_std trainf1_std trainauc_std traingmean_std trainaupr_std time_std];
                res.trainstd = train_result2;
                
                testacc_mean = mean(accuracy2); testauc_mean = mean(auc2); testerrorrate_mean = mean(errorrate2);
                testf1_mean = mean(f12); testgmean_mean = mean(gmean2); testaupr_mean = mean(aupr2);
                testsen_mean = mean(sen2); testspe_mean = mean(spe2); testrec_mean = mean(rec2); testpre_mean = mean(pre2); time_mean = mean(time);
                test_result1 = [testsen_mean testspe_mean testrec_mean testpre_mean testerrorrate_mean testacc_mean testf1_mean testauc_mean testgmean_mean testaupr_mean time_mean];
                res.testmean = test_result1;
                
                testacc_std = std(accuracy2); testauc_std = std(auc2); testerrorrate_std = std(errorrate2);
                testf1_std = std(f12); testgmean_std = std(gmean2); testaupr_std = std(aupr2);
                testsen_std = std(sen2); testspe_std = std(spe2); testrec_std = std(rec2); testpre_std = std(pre2); time_std = std(time);
                test_result2 = [testsen_std testspe_std testrec_std testpre_std testerrorrate_std testacc_std testf1_std testauc_std testgmean_std testaupr_std time_std];
                res.teststd = test_result2;
                
                clear   train_result1 train_result2 test_result1 test_result2;
                res.performance = {'sensitivity','specificity', 'recall', 'precision', 'errorrate', 'accuracy', 'f1',  'auc', 'gmean', 'aupr', 'time'};
                res_all{data_num,kk} = res;
                kk = kk+1;
                save(sprintf('%s%d%s','vttest',data_num,'.mat'),'res_all')
            end
        end
    end
end
% end
save('vt_test_result.mat','res_all');
