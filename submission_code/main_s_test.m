...
for data_num = 1:22
    ...
    kk = 1;
    s_array = 0.1:0.1:1;
    for num_s = 1:length(s_array)
        s_value = s_array(num_s);
        for f = 1:5
            ...
            for t = 1:length(t_array)
                for sigm = 1:length(sigm_array)
                    for v = 1:length(v_array)
                        tic
                        count = count + 1;
                        [data_num f v t sigm num_s]
                        kernel = Kernel('type','gaussian','gamma',sigm_array(sigm));
                        theta_value = double(rand(1,size(train_data,1)) > 0.5)';
                        theta_value = theta_value.*(-1/(v_array(v)*size(train_data,1)));
                        pinballsvddParmeter = struct('nu',v_array(v),'tao',t_array(t),'svalue',s_value, 'theta', theta_value, 'kernelFunc',kernel);
                        ...
                        val_nu(temp_ind) = v_array(t); val_tao(temp_ind) = t_array(t); val_s(temp_ind) = s_value;
                        %%
